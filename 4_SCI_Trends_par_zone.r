...
all=fread("climatic_indexes_by_grid_cell_by_year.txt",sep="\t",head=T) #climatic data
res=fread("number_data_by_grid_cell_by_year_coord_round_to_10km.txt",sep="\t",head=T) #plant records
#merge plant records and climatic data:
res=merge(all,res,by=c("Longitude2","Latitude2","year"))
res=res[!is.na(res$speciesKey),]
#make sure there is no data before 1951:
res=res[res$year>1950,]
all=all[all$year>1950,]
#make sure class of climatic variables is numeric:
...
#calculate ration between the number of reocrds by grid celle by year by species and the total number of plant records by grid celle by year:
...
fwrite(liste,"climatic_niche_trends_by_region.txt",sep="\t",row.names=F)
