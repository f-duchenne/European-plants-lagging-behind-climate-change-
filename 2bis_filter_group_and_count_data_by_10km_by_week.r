...
tab$week=week(as.Date(paste(tab$year,tab$month,tab$day,sep="-")))
...
tab3=tab %>% dplyr::group_by(speciesKey,week,year,Longitude2,Latitude2) %>% dplyr::count()
...
fwrite(res,paste0("number_data_by_grid_cell_by_week_coord_round_to_10km.txt"),sep="\t",row.names=F)

####################################################

res2=res %>% dplyr::group_by(year,Longitude2,Latitude2) %>% dplyr::summarise(nbvisit=length(unique(week)))
hist(nbvisit)